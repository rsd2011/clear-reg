...
class HrBatchQueryServiceTest {

    @Mock
    private HrBatchRepository repository;

    private HrBatchQueryService service;

    @BeforeEach
    void setUp() {
        service = new HrBatchQueryService(repository);
    }
    ...
    void givenLatestBatch_whenQuery_thenReturnOptional() {
        ...
        Scenario.given("최신 배치 조회", () -> service)
                .when("latestBatch 호출", HrBatchQueryService::latestBatch)
