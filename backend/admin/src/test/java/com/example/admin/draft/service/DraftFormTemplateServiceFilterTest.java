...
class DraftFormTemplateServiceFilterTest {
    ...
    @Test
    @DisplayName("양식 템플릿은 activeOnly=false이면 모든 템플릿을 반환한다")
    void returnsAllTemplatesWhenActiveOnlyFalse() {
        OffsetDateTime now = OffsetDateTime.now();
        DraftFormTemplateRoot root = DraftFormTemplateRoot.create(now);
        DraftFormTemplate active = createPublishedTemplate(root, "f1", WorkType.GENERAL, true, now);
        DraftFormTemplate inactive = createPublishedTemplate(root, "f2", WorkType.GENERAL, false, now);
        ...
        List<DraftFormTemplateResponse> filtered = service.listDraftFormTemplates(WorkType.GENERAL, false, ctx, false);

        assertThat(filtered).extracting(DraftFormTemplateResponse::name).containsExactlyInAnyOrder("f1", "f2");
    }
