 ...
 * 기안 템플릿 프리셋.
 *
 * <p>기안 생성 시 양식(DraftFormTemplate)과 승인선(ApprovalTemplateRoot)을 조합하여
 * 미리 구성된 프리셋을 제공한다.</p>
 */
@Entity
...
public class DraftTemplatePreset extends PrimaryKeyEntity {
     ...
     * @param contentTemplate         내용 템플릿
     * @param formTemplate            양식 템플릿
     * @param defaultApprovalTemplate 기본 승인선 템플릿
     * @param defaultFormPayload      기본 양식 페이로드
     * @param variablesJson           변수 JSON
     * @param active                  활성화 여부
     * @param now                     생성 시점
     * @return 새로운 프리셋 인스턴스
     */
     ...
     *
     * @param name                    새 이름
     * @param titleTemplate           새 제목 템플릿
     * @param contentTemplate         새 내용 템플릿
     * @param formTemplate            새 양식 템플릿
     * @param defaultApprovalTemplate 새 승인선 템플릿
     * @param defaultFormPayload      새 기본 페이로드
     * @param variablesJson           새 변수 JSON
     * @param active                  활성화 여부
     * @param now                     수정 시점
     */
     ...
     * @throws IllegalStateException 조직별 프리셋인데 다른 조직에서 접근 시
     */
    public void assertOrganization(String organizationCode) {
        if (scope == TemplateScope.ORGANIZATION && !this.organizationCode.equals(organizationCode)) {
            throw new IllegalStateException("조직별 템플릿 프리셋이 아닙니다.");
        }
    }
