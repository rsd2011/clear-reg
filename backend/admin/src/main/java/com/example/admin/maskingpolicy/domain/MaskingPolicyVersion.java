...
/**
 * 마스킹 정책 버전 엔티티 (SCD Type 2 버전 관리).
 * <p>
 * 정책의 각 버전을 저장하여 시점 조회와 감사 추적을 지원합니다.
 * 기존 MaskingPolicy의 비즈니스 데이터를 포함합니다.
 * </p>
 */
@Entity
@Table(name = "masking_policy_versions", indexes = {
        @Index(name = "idx_mpv_root_version", columnList = "root_id, version"),
        @Index(name = "idx_mpv_valid_range", columnList = "root_id, valid_from, valid_to"),
        @Index(name = "idx_mpv_feature_action", columnList = "feature_code, action_code"),
        @Index(name = "idx_mpv_priority", columnList = "priority")
})
@Getter
...
public class MaskingPolicyVersion extends PrimaryKeyEntity {
    ...
    private MaskingPolicyRoot root;
     ...
     * false: 마스킹 해제 (화이트리스트)
     */
    @Column(name = "masking_enabled", nullable = false)
    ...
    private MaskingPolicyVersion(MaskingPolicyRoot root,
        ...
        this.dataKinds = dataKinds != null ? new LinkedHashSet<>(dataKinds) : new LinkedHashSet<>();
        this.maskingEnabled = maskingEnabled != null ? maskingEnabled : true;
        this.auditEnabled = auditEnabled != null ? auditEnabled : false;
    ...
    public void updateDraft(String name,
        ...
        this.orgGroupCode = orgGroupCode;
        this.dataKinds = dataKinds != null ? new LinkedHashSet<>(dataKinds) : new LinkedHashSet<>();
        this.maskingEnabled = maskingEnabled != null ? maskingEnabled : this.maskingEnabled;
        this.auditEnabled = auditEnabled != null ? auditEnabled : this.auditEnabled;
    ...
    // === 정책 매칭 로직 (기존 MaskingPolicy에서 이관) ===
