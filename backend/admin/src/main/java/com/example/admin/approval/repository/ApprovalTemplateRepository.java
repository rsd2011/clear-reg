...
public interface ApprovalTemplateRepository extends JpaRepository<ApprovalTemplate, UUID> {
    ...
    @Query("""
            ...
            """)
    Optional<ApprovalTemplate> findByRootIdAsOf(
    ...
    @Query("""
            ...
            """)
    List<ApprovalTemplate> findHistoryByRootId(@Param("rootId") UUID rootId);

    /**
     * 버전 이력 조회 (최신순, Draft 포함).
     */
    List<ApprovalTemplate> findByRootIdOrderByVersionDesc(UUID rootId);

    /**
     * 특정 버전 번호로 조회.
     */
    Optional<ApprovalTemplate> findByRootIdAndVersion(UUID rootId, Integer version);

    /**
     * 최신 버전 번호 조회.
     */
    @Query("SELECT COALESCE(MAX(v.version), 0) FROM ApprovalTemplate v WHERE v.root.id = :rootId")
     ...
     */
    @Query("""
            SELECT COUNT(v) FROM ApprovalTemplate v
            WHERE v.root.id = :rootId
              AND v.status != com.example.common.version.VersionStatus.DRAFT
            """)
    ...
    @Query("""
            ...
            WHERE v.root.id = :rootId
              AND v.status = com.example.common.version.VersionStatus.DRAFT
            """)
    Optional<ApprovalTemplate> findDraftByRootId(@Param("rootId") UUID rootId);
    ...
    @Query("""
            ...
            """)
    List<ApprovalTemplate> findVersionsForComparison(
            @Param("rootId") UUID rootId,
