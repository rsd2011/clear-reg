 ...
 */
public interface MaskingPolicyVersionRepository extends JpaRepository<MaskingPolicyVersion, UUID> {

    /**
     * 현재 유효한 게시 버전 조회 (valid_to IS NULL AND status = PUBLISHED).
     */
    @Query("""
            SELECT v FROM MaskingPolicyVersion v
            WHERE v.root.id = :rootId
              AND v.validTo IS NULL
              AND v.status = com.example.common.version.VersionStatus.PUBLISHED
            """)
    Optional<MaskingPolicyVersion> findCurrentByRootId(@Param("rootId") UUID rootId);
    ...
    @Query("""
            ...
            """)
    Optional<MaskingPolicyVersion> findByRootIdAsOf(
    ...
    @Query("""
            ...
            """)
    List<MaskingPolicyVersion> findHistoryByRootId(@Param("rootId") UUID rootId);

    /**
     * 버전 이력 조회 (최신순, Draft 포함).
     */
    List<MaskingPolicyVersion> findByRootIdOrderByVersionDesc(UUID rootId);

    /**
     * 특정 버전 번호로 조회.
     */
    Optional<MaskingPolicyVersion> findByRootIdAndVersion(UUID rootId, Integer version);

    /**
     * 최신 버전 번호 조회.
     */
    @Query("SELECT COALESCE(MAX(v.version), 0) FROM MaskingPolicyVersion v WHERE v.root.id = :rootId")
     ...
     */
    @Query("""
            SELECT COUNT(v) FROM MaskingPolicyVersion v
            WHERE v.root.id = :rootId
              AND v.status != com.example.common.version.VersionStatus.DRAFT
            """)
    ...
    @Query("""
            ...
            WHERE v.root.id = :rootId
              AND v.status = com.example.common.version.VersionStatus.DRAFT
            """)
    Optional<MaskingPolicyVersion> findDraftByRootId(@Param("rootId") UUID rootId);
    ...
    @Query("""
            ...
            """)
    List<MaskingPolicyVersion> findVersionsForComparison(
            @Param("rootId") UUID rootId,
    ...
    @Query("""
            ...
            """)
    List<MaskingPolicyVersion> findAllCurrentActiveVersions();
}
