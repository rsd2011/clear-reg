...
public class JitProvisioningService {
   ...
   *
   * @param loginTypeName 로그인 타입 이름 (SSO, AD, PASSWORD 등)
   */
  public boolean isEnabledFor(String loginTypeName) {
    if (!properties.isEnabled()) {
      return false;
    }
    Set<String> enabledTypes = properties.getEnabledLoginTypes();
    return enabledTypes.isEmpty() || enabledTypes.contains(loginTypeName);
  }
  ...
  public JitProvisioningResult provisionForAd(String username, String adDomain) {
    ...
    // 2. 신규 사용자 생성
    return createNewUser(username, null, adDomain);
  }
