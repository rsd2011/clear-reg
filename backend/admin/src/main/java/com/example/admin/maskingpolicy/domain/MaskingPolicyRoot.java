 ...
 * 마스킹 정책 루트 엔티티.
 * <p>
 * 버전 컨테이너 역할을 하며, 실제 비즈니스 데이터는 {@link MaskingPolicyVersion}에 저장됩니다.
...
public class MaskingPolicyRoot extends PrimaryKeyEntity {
    ...
    private MaskingPolicyVersion currentVersion;

    /** 이전 활성 버전 (롤백 참조용) */
    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "previous_version_id")
    private MaskingPolicyVersion previousVersion;

    /** 다음 예약 버전 (Draft 또는 결재 대기용) */
    @OneToOne(fetch = FetchType.LAZY)
    @JoinColumn(name = "next_version_id")
    private MaskingPolicyVersion nextVersion;
     ...
     * @param now        활성화 시점
     */
    public void activateNewVersion(MaskingPolicyVersion newVersion, OffsetDateTime now) {
     ...
     */
    public void setDraftVersion(MaskingPolicyVersion draftVersion) {
    ...
    public Integer getPriority() {
        return currentVersion != null ? currentVersion.getPriority() : null;
    }
