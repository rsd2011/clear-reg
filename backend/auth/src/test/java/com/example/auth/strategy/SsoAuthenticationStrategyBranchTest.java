...
class SsoAuthenticationStrategyBranchTest {
    ...
    void nullTokenThrows() {
        SsoAuthenticationStrategy strategy = new SsoAuthenticationStrategy(mock(SsoClient.class), mock(UserAccountService.class));

        assertThatThrownBy(() -> strategy.authenticate(new LoginRequest(LoginType.SSO, null, null, null)))
    ...
    void resolvesAndFetchesUser() {
        ...
        given(userService.getByUsernameOrThrow("alice")).willReturn(UserAccount.builder()
                .username("alice").password("p").organizationCode("ORG").permissionGroupCode("PG").build());
