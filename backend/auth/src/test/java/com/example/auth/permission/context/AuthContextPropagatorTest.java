...
class AuthContextPropagatorTest {
    ...
    void runWithCurrentContext() {
        ...
        Runnable wrapped = AuthContextPropagator.wrapCurrentContext(() ->
    ...
    void callWithContext() throws Exception {
        AuthContext system = new AuthContext("system", "ROOT", "AUDIT",
                FeatureCode.AUDIT_LOG, ActionCode.READ, RowScope.ALL, java.util.Map.of());
        AuthContextHolder.set(new AuthContext("caller", "ORG", "DEFAULT",
                FeatureCode.NOTICE, ActionCode.READ, RowScope.OWN, java.util.Map.of()));

        Callable<String> callable = AuthContextPropagator.wrap(() ->
