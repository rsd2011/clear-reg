...
class PermissionEvaluatorCheckFailureTest {
    ...
    void permissionCheckFailurePropagates() {
        ...
        PermissionEvaluator evaluator = new PermissionEvaluator(userAccountService, groupService, orgPolicyService, java.util.List.of(check));
        ...
        Mockito.doThrow(new PermissionDeniedException("denied"))
                .when(check).check(Mockito.any(PermissionEvaluationContext.class));
