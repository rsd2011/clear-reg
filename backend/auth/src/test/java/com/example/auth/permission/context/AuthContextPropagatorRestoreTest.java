...
class AuthContextPropagatorRestoreTest {
    ...
    void runWithContextRestoresPrevious() {
        AuthContext previous = new AuthContext("prev", "ORG", "PG", FeatureCode.ORGANIZATION, ActionCode.READ, RowScope.OWN, Map.of());
        AuthContextHolder.set(previous);

        AuthContext newCtx = new AuthContext("new", "ORG", "PG", FeatureCode.ORGANIZATION, ActionCode.UPDATE, RowScope.ALL, Map.of());

        AuthContextPropagator.runWithContext(newCtx, () -> {
