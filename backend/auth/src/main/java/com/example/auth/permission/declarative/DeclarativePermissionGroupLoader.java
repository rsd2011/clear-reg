...
class DeclarativePermissionGroupLoader implements ApplicationRunner {
    ...
    DeclarativePermissionGroupLoader(PermissionGroupRepository repository,
    ...
    public void run(ApplicationArguments args) {
        ...
        try (InputStream inputStream = resource.getInputStream()) {
            PermissionCatalogDefinition catalog = yamlMapper.readValue(inputStream, PermissionCatalogDefinition.class);
    ...
    void synchronize(List<PermissionGroupDefinition> definitions) {
        for (PermissionGroupDefinition definition : definitions) {
            ...
            PermissionGroup group = repository.findByCode(definition.code())
                    .orElseGet(() -> new PermissionGroup(definition.code(), definition.name()));

            group.updateDetails(definition.name(),
    ...
    private Set<PermissionAssignment> toAssignments(List<PermissionAssignmentDefinition> definitions) {
        ...
        for (PermissionAssignmentDefinition definition : definitions) {
            ...
            assignments.add(new PermissionAssignment(definition.feature(), definition.action(), rowScope, condition));
        }
        return assignments;
    }
