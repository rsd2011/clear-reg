...
public class AuditPortPermissionAuditLogger implements PermissionAuditLogger {
    ...
    public void onAccessDenied(AuthContext context, Throwable throwable) {
        record("PERMISSION_DENIED", context, false, throwable != null ? throwable.getClass().getSimpleName() : "DENIED");
    }
    ...
    private void record(String action, AuthContext ctx, boolean success, String resultCode) {
        try {
            AuditEvent event = AuditEvent.builder()
                    ...
                    .extraEntry("action", ctx != null && ctx.action() != null ? ctx.action().name() : null)
                    .extraEntry("rowScope", ctx != null && ctx.rowScope() != null ? ctx.rowScope().name() : null)
                    .build();
