...
class FileServiceScanTest {
    ...
    private FileService service() {
        PolicyToggleSettings settings = new PolicyToggleSettings(false, false, false, List.of(), 20, List.of("txt"), true, 365);
    ...
    @Test
    @DisplayName("스캔 결과가 BLOCKED이면 업로드를 중단하고 예외를 던진다")
    void uploadFailsWhenScanBlocked() throws Exception {
        byte[] payload = "evil".getBytes();
        given(storageClient.store(org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.anyLong(), org.mockito.ArgumentMatchers.anyString()))
                .willReturn(new FileStorageClient.StoredObject("path", payload.length));
        given(fileScanner.scan(org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.any()))
                .willReturn(ScanStatus.BLOCKED);
    ...
    @Test
    @DisplayName("스캔 중 예외가 발생하면 FAILED로 간주해 업로드를 차단한다")
    void uploadFailsWhenScanThrows() throws Exception {
        byte[] payload = "data".getBytes();
        given(storageClient.store(org.mockito.ArgumentMatchers.any(), org.mockito.ArgumentMatchers.anyLong(), org.mockito.ArgumentMatchers.anyString()))
                .willReturn(new FileStorageClient.StoredObject("path", payload.length));
        given(fileScanner.scan(org.mockito.ArgumentMatchers.anyString(), org.mockito.ArgumentMatchers.any()))
                .willThrow(new RuntimeException("scanner down"));
