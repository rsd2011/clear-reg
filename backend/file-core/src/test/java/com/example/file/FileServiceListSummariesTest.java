...
class FileServiceListSummariesTest {
    ...
    @Test
    @DisplayName("요약 목록을 조회하면 리포지토리 결과를 그대로 반환한다")
    void listSummariesReturnsRepositoryResult() {
        FileSummaryView view = mock(FileSummaryView.class);
        given(storedFileRepository.findAllByOrderByCreatedAtDesc()).willReturn(List.of(view));

        List<FileSummaryView> result = service().listSummaries();

        assertThat(result).containsExactly(view);
        verify(storedFileRepository).findAllByOrderByCreatedAtDesc();
    }
}
