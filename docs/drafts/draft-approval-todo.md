...
| T7 | 서비스 | 참조자 등록 및 알림 트리거 | High | ✅ 구현: 생성/상신 시 참조자 등록 및 이벤트 발행 |
...
| T17 | 파일 | 첨부 다운로드/삭제 정책 강화 | Medium | 진행: 스캔 포트/메타 필드/마이그레이션 완료, FileAuditPublisher 훅(log/kafka/outbox/siem) + outbox 릴레이 추가, 다운로드에 draftId 컨텍스트 검증 추가, 스캔 설정(max-size/timeout/toggle)+재스캔 스케줄러 적용, 정책 문서 업데이트. 다음: 결재자/참조자 권한 체크 세부 검증, 서명 URL TTL 적용, 스캔 보존 TTL 정책 보완 |
## Document Follow-ups
- `docs/security/permissions/permissions.md`: 결재자/참조자 권한, 상태 전이 정책 추가 (T4, T5, T14)
- `docs/architecture-todo.md`: 아키텍처 전반 TODO에 본 모듈 링크 추가, Nuxt4 프런트 전환 계획 반영
- `docs/operations/observability/overview.md` + OTEL 설정: 결재 리드타임/반려율 등 비즈니스 메트릭 노출 (T12, T16)
