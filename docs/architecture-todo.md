...
- [ ] 기안/결재/템플릿/참조/알림 백로그 정리 및 실행
  ...
  - 현재 완료: T1(업무-템플릿 매핑), T2(참조자), T3(결재 그룹 멤버십), T4(결재자 검증+락), T5(회수/재상신/위임), T6(기본 템플릿 자동 선택), T7(알림 퍼블리셔/채널 플러그형), T8(템플릿/그룹 CRUD API), T10(Nuxt4 SPA용 OpenAPI 초안). 프런트는 Nuxt4로 전환 예정 → Thymeleaf 제거 및 API 계약 정비 완료. 다음: T9, T11~T12 이후 항목 실행.
